...
import { SessionKeyValidatorABI } from "@0xknwn/starknet-module-sessionkey";
...
describe.each([data[0]])(
  ...
  ({ fees, accountID, version }) => {
    ...
    it(
      `[${fees}]checks the sessionkey is disabled`,
      async () => {
        const conf = config(env);
        const calldata = new CallData(SessionKeyValidatorABI);
        const sessionkey = await sessionKeyModule.get_session_key();
    ...
    it(
      `[${fees}]increments the counter from SmartrAccount with Module and fails`,
      async () => {
        if (!counterContract) {
          throw new Error("Counter not deployed");
        }
