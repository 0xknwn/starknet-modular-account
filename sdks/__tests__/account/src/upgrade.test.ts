  ...
  ({ fees, version, accountID }) => {
    ...
    it(
      ...
      async () => {
        ...
        expect(num.toHexString(num.toBigInt(c))).toEqual(
          num.toHexString(num.toBigInt(classHash(classNames.SmartrAccount)))
        );
      },
      default_timeout
    );
    ...
    it(
      ...
      async () => {
        ...
        expect(`0x${c.toString(16)}`).toEqual(
          conf.accounts[accountID].publicKey
        );
      },
      default_timeout
