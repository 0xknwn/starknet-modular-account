...
describe.each([dataset[1], dataset[3]])(
  ...
  ({ name, data, fees, accountID, version }) => {
    ...
    it(
      `[${fees}][${name}]: adds a module to the account`,
      async () => {
        if (!smartrAccount) {
    ...
    it(
      `[${fees}][${name}]: checks the module is installed`,
      async () => {
        if (!smartrAccount) {
          throw new Error("SmartrAccount is not deployed");
        }
        const output = await smartrAccount.isModule(classHash(data.className));
    ...
    it(`[${fees}][${name}]: creates an typescript account with the module`, async () => {
      if (!module) {
    ...
    it(
      `[${fees}][${name}]: increments the counter with the account/module and succeeds`,
      async () => {
        if (!counterContract) {
          throw new Error("Counter not deployed");
        }
    ...
    it(
      `[${fees}][${name}]: increments the counter with wrong key and fails`,
      async () => {
        if (!module) {
          expect(module).toBeDefined();
          return;
    ...
    it(
      `[${fees}][${name}]: removes the Validator module from the account`,
      async () => {
        if (!smartrAccount) {
          throw new Error("SmartrAccount is not deployed");
        }
    ...
    it(
      `[${fees}][${name}]: checks the Validator is not installed anymore`,
      async () => {
        if (!smartrAccount) {
          throw new Error("SmartrAccount is not deployed");
        }
