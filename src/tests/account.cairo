...
fn test_deploy_simple_account() {
  ...
  let computed_account_address: ContractAddress = compute_contract_address(deployerAddress, publicKey, account_class_hash, array![publicKey, 0x10]);
  let (account_address, _) = account_class.deploy_at(@array![publicKey, 0x10], computed_account_address).unwrap();
